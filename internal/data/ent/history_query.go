...
func (hq *HistoryQuery) QueryVersion() *VersionQuery {
	query := &VersionQuery{config: hq.config}
...
func (hq *HistoryQuery) Clone() *HistoryQuery {
	...
	return &HistoryQuery{
		...
		withVersion: hq.withVersion.Clone(),
...
func (hq *HistoryQuery) WithVersion(opts ...func(*VersionQuery)) *HistoryQuery {
	query := &VersionQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withVersion = query
	return hq
}
...
func (hq *HistoryQuery) sqlAll(ctx context.Context) ([]*History, error) {
	...
	if query := hq.withVersion; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*History, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Version = []*Version{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(history.VersionPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, hq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "version": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "version" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Version = append(nodes[i].Edges.Version, n)
			}
		}
	}
