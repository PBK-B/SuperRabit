...
func (uc *UserCreate) AddVersionIDs(ids ...int) *UserCreate {
	uc.mutation.AddVersionIDs(ids...)
	return uc
}

// AddVersions adds the "versions" edges to the Version entity.
func (uc *UserCreate) AddVersions(v ...*Version) *UserCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return uc.AddVersionIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.AppsIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.VersionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
