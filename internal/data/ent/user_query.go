...
func (uq *UserQuery) QueryApps() *AppQuery {
	query := &AppQuery{config: uq.config}
...
func (uq *UserQuery) QueryVersions() *VersionQuery {
	query := &VersionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withApps:     uq.withApps.Clone(),
		withVersions: uq.withVersions.Clone(),
...
func (uq *UserQuery) WithVersions(opts ...func(*VersionQuery)) *UserQuery {
	query := &VersionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withVersions = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withApps; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.AppsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "apps": %w`, err)
		}
	...
	if query := uq.withVersions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Versions = []*Version{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*User)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.VersionsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "versions": %w`, err)
		}
		query.Where(version.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "versions" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Versions = append(nodes[i].Edges.Versions, n)
			}
		}
	}
